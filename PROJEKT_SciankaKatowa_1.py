...
def oblicz_i_rysuj(d, return_figures=False, save_pdf=True):
    ...
    Jeśli `save_pdf=False`, nie wykonuje się zapisu do plików, a `path1/path2` są `None`.
    """
    pi = np.pi
    q = d["q"]
    gamma = d["gamma"]
    ...
    # żeby uniknąć niestabilności numerycznych (0/0 w K_a_gamma).
    if abs(epsilon) < 1e-6:
    ...
    scale_q = 0.25 * x_max * (q_d / max(E_ah, 1e-6))
    if scale_q > 1e-8:
    ...
    ax1.text(0.98, 0.02, txt_rankine, transform=ax1.transAxes, fontsize=9, verticalalignment="bottom",
             horizontalalignment="right", bbox=dict(boxstyle="round,pad=0.4", facecolor="white", edgecolor="gray"))
    ...
    scale_q_p = 0.25 * x_max_p * (q_d / max(E_ah_poncelet, 1e-6))
    if scale_q_p > 1e-8:
    ...
    scale_E_p = 0.25 * x_max_p / E_ah_poncelet
    ...
    ax2.text(0.98, 0.02, txt_poncelet, transform=ax2.transAxes, fontsize=9, verticalalignment="bottom",
             horizontalalignment="right", bbox=dict(boxstyle="round,pad=0.4", facecolor="white", edgecolor="gray"))
    ...
    wyniki = {
        "rankine": {"E_a": E_a, "E_ah": E_ah, "E_av": E_av, "z_c": z_c},
        "poncelet": {"E_a": E_a_poncelet, "E_ah": E_ah_poncelet, "E_av": E_av_poncelet, "z_c": z_c_poncelet},
    }
    if return_figures:
